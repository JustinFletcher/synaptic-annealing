...
@everywhere function updateState_oscillatory(stateTupleIn)
    ...
    # Set the strength of the tunneling field.
	tunnelingField = (sin(((1/100)*numEpochs)+3*rand())+1)/2
	temperature = initTemperature*(sin(((1/100)*numEpochs)+3*rand())+1)/2
	anisotropicField = 1-(sin(((1/100)*numEpochs)+3*rand())+1)/2
