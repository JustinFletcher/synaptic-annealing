...
\begin{document}
	...
	% What is the pgysical mechanism for depression?
	
	% Concentraion of nuerotransimitters in the extra-sybaptic flusid increases, raising the instantaneous membrane porentential, possibly vreaching the threshold causing a fire-and-reset, where the there is some pgysical limit on the rate at which the reset step can occur, thus causing the diminishing returns seen in Figure 2 of Neurocomputing, whence the fire causes either excitatory or depressive neurotransmitters to be released at the synaptic clefts formed at the end of the axon, thus propigatind the activation through the net.
	
	% What is changed in synaptic annealing is the effecientcy with which a partyiculat synaptic site is able to convert molecules in the surroundinf intercellulat inbitoment, which is to asay in the synaptic cleft, into a change in charge inside the cell.
	
	% See paper 23 in Neurocomputing for more information...
	...
	The perceptron model and its extensions generated a great deal of excitement in the field of neural networks. Several 
	
	In the final chapter of the \textit{Perceptrons} Minsky and Papert extended thier judgements regarding the ineffectiveness of single-layered perceptrons to all varients of perceptrons, including the multi-layered variety. This statement would turn out to be one of the most impactful of the entire book, as it likely resulted in a chilling of funding for neural network research which lasted for several years. Unfortunately, this judgement was false.
